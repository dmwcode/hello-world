...
$FileListOfUsers = "c:\scripts\disableusers.txt"
 
# Active Directory OU (unique name) that users will move to
$MoveToOU = 'OU=Disabled Accounts,DC=andoverma,DC=local'
...
$ListOfUsers | Get-ADUser | Disable-ADAccount
...
$ListOfUsers | Get-ADUser | Move-ADObject -TargetPath (Get-ADOrganizationalUnit $MoveToOu)
